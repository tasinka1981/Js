...
//  */
const getUsersByGender = (arr, gender) => {
    return (arr.filter(item => item.gender === gender).map(item => item.name))
};
...
//  */
const getTotalBalance = arr => {
    return arr.reduce((acc, item) => acc + item.balance, 0)
};
...
//  */
const getUsersByFriend = (arr, name) => {
    return arr.filter(item => item.friends.includes(name)).map(item => item.name)
};
